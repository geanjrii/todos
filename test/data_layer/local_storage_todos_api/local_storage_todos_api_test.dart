...
import 'package:todos/data_layer/local_storage_todos_api/local_storage_todos_api.dart';
...
void main() {
  group('LocalStorageTodosApi', () {
    ...
    setUp(() {
      ...
      when(() => plugin.getString(any())).thenReturn(json.encode(mockTodos));
    ...
    group('constructor', () {
      ...
      group('initializes the todos stream', () {
        test('with existing todos if present', () {
          ...
          expect(subject.getTodos(), emits(mockTodos));
          verify(
            () => plugin.getString(
    ...
    group('saveTodo', () {
      test('saves new todos', () {
        ...
        final newTodos = [...mockTodos, newTodo];
      ...
      test('updates existing todos', () {
        ...
        final newTodos = [updatedTodo, ...mockTodos.sublist(1)];
    ...
    group('deleteTodo', () {
      test('deletes existing todos', () {
        final newTodos = mockTodos.sublist(1);
    ...
    group('clearCompleted', () {
      test('deletes all completed todos', () {
        final newTodos = mockTodos.where((todo) => !todo.isCompleted).toList();
        final deletedTodosAmount = mockTodos.length - newTodos.length;
    ...
    group('completeAll', () {
      test('sets isCompleted on all todos to provided value', () {
        final newTodos =
            mockTodos.map((todo) => todo.copyWith(isCompleted: true)).toList();
        final changedTodosAmount =
            mockTodos.where((todo) => !todo.isCompleted).length;
